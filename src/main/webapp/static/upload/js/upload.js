$.fn.bindUpload = function () {
    var myData = {
        ...
        num: arguments[0].num || "",
        reviewPath:arguments[0].reviewPath || "",
        callbackPath: arguments[0].callbackPath || "#callbackPath",
        module:arguments[0].module || "default",
    ...
    this.upload = function (max, i, item) {
        ...
        formData.append("module",myData.module);
        $.ajax({
            url: myData.url,
            type: "post",
            data: formData,
            ...
            , success: function (data) {
                ...
                if (data.code == "0") {
                    ...
                    if (myData.num && i == (myData.num - 1) || i == (max - 1)) {
                        callInput.val(data.data);
                    } else {
                        callInput.val(data.data + "|");
                    }
                    var reivew = $(myData.reviewPath);
                    var host = window.location.host;
                    var pathName=window.document.location.pathname;
                    var projectName=pathName.substring(0,pathName.substr(1).indexOf('/')+1);
                    var http =  'https:' == document.location.protocol ? "https://": "http://";
                    $(reivew).attr("src",http+host+projectName+"/"+data.data);
                } else {
                    itemList.eq(i).find(".state").html(1);
                    itemList.eq(i).find(".err").width("100%").html("上传失败");
                }
